...
export async function POST(request: Request) {
  ...
  const { count } = await supabase
    ...
    .eq('user_id', user.id)
    .like('number', `DEV-${year}-%`);

  const number = nextDocumentNumber('DEV', year, count ?? 0);
  ...
  const insertPayload = {
    ...
    expiry_date: parsed.data.expiry_date && parsed.data.expiry_date !== '' ? parsed.data.expiry_date : null,
