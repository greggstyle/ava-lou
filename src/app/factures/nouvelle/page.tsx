...
function NouvelleFactureForm() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      // Prefill client (match by name if exists)
      ...
      if (Array.isArray(entities.line_items) && entities.line_items.length > 0) {
        ...
        // Pick a vat rate from the first line, fallback 20
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: C.bone }}>
      <AvaTopBar title="Nouvelle facture" onBack={() => router.back()} />
      ...
      <form onSubmit={onSubmit} style={{ padding: '8px 20px 120px', flex: 1, overflowY: 'auto' }}>
        ...
        <AvaCard padding={18} style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <AvaField label="Date d'échéance">
              <input
                style={inputStyle}
                type="date"
                value={dueDate}
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
          <Link href="/factures" style={{ textDecoration: 'none', flex: 1 }}>
